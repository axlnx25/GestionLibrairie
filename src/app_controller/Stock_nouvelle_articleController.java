...
public class Stock_nouvelle_articleController implements Initializable {
    ...
    private void bouton_filtre(ActionEvent event) throws SQLException {
        ...
        if (okFiltre) {
            ...
            // On met à jour la table sans modifier la liste originale pendant la boucle
            stock_tableview.setItems(articlesFiltres);

            valeur_stock_label.setText("VALEUR STOCK: " + valeurStock(articlesFiltres));
        } else {
            // alerte combo vide
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("SELECTION INVALIDE");
            alert.setHeaderText(null);
            alert.setContentText("Veuillez selectionner un type article valide.");
            alert.showAndWait();
        }
    }
    ...
    private void ajouter_type_article(ActionEvent event) throws IOException {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/app_fxml/type_article.fxml"));
        Parent root = loader.load();
        Type_articleController ctrl = loader.getController();
        ctrl.setAllDAO(venteDAO, depenseDAO, approvisionnementDAO, articleDAO, typeArticleDAO, factureDAO, utilisateurDAO);
        try { ctrl.chargerTypeArticle(); } catch (SQLException ignored) {}
        Stage stage = (Stage) prix_vente_textfield.getScene().getWindow();
        stage.setScene(new Scene(root));
    }
    ...
    private void ajouter_article(ActionEvent event) throws SQLException {
        ...
        if (okArticle && okQuantite && okPrixVente && okType) {
            ...
            valeur_stock_label.setText("VALEUR STOCK: " + valeurStock(stock_tableview.getItems()));
        } else {
    ...
    private void supprimer_article(ActionEvent event) throws SQLException {
        ...
        if (selection != null) {
            ...
            valeur_stock_label.setText("VALEUR STOCK: " + valeurStock(articleObservableList));
        } else {
    ...
    public String valeurStock(ObservableList<Article> articleObservableList) throws SQLException {
        int somme = 0;
        for (Article article : articleObservableList) {
            somme += (article.getPrixVenteArticle() * article.getQuantiteArticle());
        }
